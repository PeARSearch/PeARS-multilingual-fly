...
Usage:
  wiki_process.py --lang=<s> --spm=<path> (--get_pages|--get_categories)
  wiki_process.py (-h | --help)
  wiki_process.py --version

Options:
  --lang=<s>         The language of the Wikipedia to process.
  --spm=<path>       The path to the sentencepiece model to use (e.g. spm/enwiki.model).
...
def retrieve_page_content(num_page_per_cat):
    metacats = glob("./data/"+lang+"/categories/*")

    for datacat_dir in metacats:
        print("--- Processing category",datacat_dir,"---")
        title_file = join(datacat_dir,"titles.txt")
        ...
        content_file = open(join(datacat_dir,"linear.txt"),'w')
        ...
        for i in range(max_titles):
            ...
            cat = datacat_dir.replace("./data/"+lang+"/categories/","")
...
if __name__ == '__main__':
    ...
    if args['--get_pages']:
        while True:
            ...
            if user_input != 'q':
                get_wiki_page_titles(user_input)
                retrieve_page_content(200) #200 pages per cat
            else:
                break
